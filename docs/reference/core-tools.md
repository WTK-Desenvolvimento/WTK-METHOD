...
Self-hosted **n8n** skills (`wtk-n8n-*`) are **not** part of core — they ship in the optional built-in **n8n** module. Select **WTK n8n (self-hosted)** during `npx wtk-method install`, then see [Official Modules](./modules.md#optional-built-in-n8n-self-hosted).
