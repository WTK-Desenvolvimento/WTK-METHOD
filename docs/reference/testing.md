...
Wtk provides two testing paths: a built-in QA agent for fast test generation and an installable Test Architect module for enterprise-grade test strategy.
...
| **Setup** | Nothing to install -- included in BMM | Install separately via `npx wtk-method install` |
...
**Trigger:** `QA` or `wtk-qa-generate-e2e-tests`
...
- **Documentation:** [TEA Module Docs](https://wtk-code-org.github.io/wtk-method-test-architecture-enterprise/)
- **Install:** `npx wtk-method install` and select the TEA module
...
Quinn's Automate workflow appears in Phase 4 (Implementation) of the Wtk Method workflow map. It is designed to run **after a full epic is complete** — once all stories in an epic have been implemented and code-reviewed. A typical sequence:
...
3. Run retrospective (`wtk-retrospective`) to capture lessons learned
