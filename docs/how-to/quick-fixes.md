...
Use **Quick Dev** for bug fixes, refactorings, or small targeted changes that don't require the full Wtk Method.
...
```text
run quick-dev — implement the intent in _wtk-output/implementation-artifacts/my-intent.md
```
...
Consider using the full Wtk Method when:
...
See [Quick Dev](../explanation/quick-dev.md) for more on how Quick Dev fits into the Wtk Method.
