...
description: Install Wtk using command-line flags for CI/CD pipelines and automated deployments
sidebar:
  order: 2
---

Use command-line flags to install Wtk non-interactively. This is useful for:
...
| `--output-folder <path>` | Output folder path | _wtk-output |
...
- `bmm` — Wtk Method Master
- `bmb` — Wtk Builder

Check the [Wtk registry](https://github.com/wtk-code-org) for available external modules.
...
Run `npx wtk-method install` interactively once to see the full current list of supported tools, or check the [platform codes configuration](https://github.com/wtk-code-org/WTK-METHOD/blob/main/tools/cli/installers/lib/ide/platform-codes.yaml).

## Installation Modes

| Mode | Description | Example |
|------|-------------|---------|
| Fully non-interactive | Provide all flags to skip all prompts | `npx wtk-method install --directory . --modules bmm --tools claude-code --yes` |
| Semi-interactive | Provide some flags; Wtk prompts for the rest | `npx wtk-method install --directory . --modules bmm` |
| Defaults only | Accept all defaults with `-y` | `npx wtk-method install --yes` |
| Without tools | Skip tool/IDE configuration | `npx wtk-method install --modules bmm --tools none` |
...
- A fully configured `_wtk/` directory in your project
- Compiled agents and workflows for your selected modules and tools
- A `_wtk-output/` folder for generated artifacts

## Validation and Error Handling

Wtk validates all provided flags:
...
:::note[Still stuck?]
Run with `--debug` for detailed output, try interactive mode to isolate the issue, or report at <https://github.com/wtk-code-org/WTK-METHOD/issues>.
:::
