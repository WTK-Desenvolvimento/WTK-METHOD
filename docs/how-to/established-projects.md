...
description: How to use Wtk Method on existing codebases
sidebar:
  order: 6
---

Use Wtk Method effectively when working on existing projects and legacy codebases.

This guide covers the essential workflow for onboarding to existing projects with Wtk Method.

:::note[Prerequisites]
- Wtk Method installed (`npx wtk-method install`)
...
If you have completed all PRD epics and stories through the Wtk process, clean up those files. Archive them, delete them, or rely on version history if needed. Do not keep these files in:
...
You can review and refine the generated file, or create it manually at `_wtk-output/project-context.md` if you prefer.
...
For complex projects, consider using the `wtk-document-project` workflow. It offers runtime variants that will scan your entire project and document its actual current state.

## Step 3: Get Help

### Wtk-Help: Your Starting Point

**Run `wtk-help` anytime you're unsure what to do next.** This intelligent guide:
...
```
wtk-help I have an existing Rails app, where should I start?
wtk-help What's the difference between quick-flow and full method?
wtk-help Show me what workflows are available
```

Wtk-Help also **automatically runs at the end of every workflow**, providing clear guidance on exactly what to do next.
...
| ------------------------------ | ----------------------------------------------------------------------------------------------------------------------------- |
| **Small updates or additions** | Run `wtk-quick-dev` to clarify intent, plan, implement, and review in a single workflow. The full four-phase Wtk Method is likely overkill. |
| **Major changes or additions** | Start with the Wtk Method, applying as much or as little rigor as needed.                                                    |
