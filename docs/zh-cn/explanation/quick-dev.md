...
`wtk-quick-dev` 重新平衡了这种权衡。它信任模型在更长的时间段内无监督运行，但前提是工作流已经创建了足够强的边界来确保安全。
...
意图可以以多种形式出现：几句话、一个错误追踪器链接、计划模式的输出、从聊天会话复制的文本，甚至来自 WTK 自己的 `epics.md` 的故事编号。在最后一种情况下，工作流不会理解 WTK 故事跟踪语义，但它仍然可以获取故事本身并继续执行。
