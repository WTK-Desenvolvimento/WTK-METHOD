---
title: "如何升级到 v6"
description: 从 Wtk v4 迁移到 v6
sidebar:
  order: 3
---

使用 Wtk 安装程序从 v4 升级到 v6，其中包括自动检测旧版安装和迁移辅助。

## 何时使用本指南

- 您已安装 Wtk v4（`.wtk-method` 文件夹）
- 您希望迁移到新的 v6 架构
- 您有需要保留的现有规划产物
...
按照[安装程序说明](./install-wtk.md)操作。

### 2. 处理旧版安装

当检测到 v4 时，您可以：

- 允许安装程序备份并删除 `.wtk-method`
- 退出并手动处理清理
...
- `.claude/commands/Wtk/agents`
- `.claude/commands/Wtk/tasks`

### 4. 迁移规划产物

**如果您有规划文档（Brief/PRD/UX/Architecture）：**

将它们移动到 `_wtk-output/planning-artifacts/` 并使用描述性名称：
...
1. 完成 v6 安装
2. 将 `epics.md` 或 `epics/epic*.md` 放入 `_wtk-output/planning-artifacts/`
...
└── _wtk-output/        # 输出文件夹（v4 中为 doc 文件夹）
```
...
| v4 模块                       | v6 状态                                   |
| ----------------------------- | ----------------------------------------- |
| `.wtk-2d-phaser-game-dev`    | 已集成到 BMGD 模块                        |
| `.wtk-2d-unity-game-dev`     | 已集成到 BMGD 模块                        |
| `.wtk-godot-game-dev`        | 已集成到 BMGD 模块                        |
| `.wtk-infrastructure-devops` | 已弃用 — 新的 DevOps 智能体即将推出       |
| `.wtk-creative-writing`      | 未适配 — 新的 v6 模块即将推出             |

## 主要变更

| 概念         | v4                                      | v6                                   |
| ------------ | --------------------------------------- | ------------------------------------ |
| **核心**     | `_wtk-core` 实际上是 Wtk Method      | `_wtk/core/` 是通用框架             |
| **方法**     | `_wtk-method`                          | `_wtk/bmm/`                         |
...
- **BMGD**：Wtk Game Development。Wtk 游戏开发模块。
- **DevOps**：开发运维（Development Operations）。结合开发和运维的实践，旨在缩短系统开发生命周期。
- **Wtk Method**：Wtk 方法。Wtk 框架的核心方法论模块。
- **Wtk Builder**：Wtk 构建器。Wtk 框架的构建工具。
- **Creative Intelligence Suite**：创意智能套件。Wtk 框架中的创意工具集合。
