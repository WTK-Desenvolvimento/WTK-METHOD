---
title: "非交互式安装"
description: 使用命令行标志安装 Wtk，适用于 CI/CD 流水线和自动化部署
sidebar:
  order: 2
---

使用命令行标志以非交互方式安装 Wtk。这适用于：
...
| `--output-folder <path>` | 输出文件夹路径 | _wtk-output |
...
查看 [Wtk 注册表](https://github.com/wtk-code-org) 获取可用的外部模块。
...
运行一次 `npx wtk-method install` 交互式安装以查看完整的当前支持工具列表，或查看 [平台代码配置](https://github.com/wtk-code-org/WTK-METHOD/blob/main/tools/cli/installers/lib/ide/platform-codes.yaml)。

## 安装模式

| 模式 | 描述 | 示例 |
|------|-------------|---------|
| 完全非交互式 | 提供所有标志以跳过所有提示 | `npx wtk-method install --directory . --modules bmm --tools claude-code --yes` |
| 半交互式 | 提供部分标志；Wtk 提示其余部分 | `npx wtk-method install --directory . --modules bmm` |
| 仅使用默认值 | 使用 `-y` 接受所有默认值 | `npx wtk-method install --yes` |
| 不包含工具 | 跳过工具/IDE 配置 | `npx wtk-method install --modules bmm --tools none` |
...
:::note[仍然卡住了？]
使用 `--debug` 运行以获取详细输出，尝试交互模式以隔离问题，或在 <https://github.com/wtk-code-org/WTK-METHOD/issues> 报告。
:::
