...
`wtk-quick-dev` rebalances that tradeoff. It trusts the model to run unsupervised for longer stretches, but only after the workflow has created a strong enough boundary to make that safe.
...
Intent can come in many forms: a couple of phrases, a bug tracker link, output from plan mode, text copied from a chat session, or even a story number from WTK's own `epics.md`. In that last case, the workflow will not understand WTK story-tracking semantics, but it can still take the story itself and run with it.
