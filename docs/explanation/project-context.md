...
- `wtk-create-architecture` — respects technical preferences during solutioning
- `wtk-create-story` — informs story creation with project patterns
- `wtk-dev-story` — guides implementation decisions
- `wtk-code-review` — validates against project standards
- `wtk-quick-dev` — applies patterns when implementing tech-specs
- `wtk-sprint-planning`, `wtk-retrospective`, `wtk-correct-course` — provides project-wide context
...
|----------|----------------|---------|
| **New project, before architecture** | Manually, before `wtk-create-architecture` | Document your technical preferences so the architect respects them |
| **New project, after architecture** | Via `wtk-generate-project-context` or manually | Capture architecture decisions for implementation agents |
| **Existing project** | Via `wtk-generate-project-context` | Discover existing patterns so agents follow established conventions |
| **Quick Flow project** | Before or during `wtk-quick-dev` | Ensure quick implementation respects your patterns |
...
Create the file at `_wtk-output/project-context.md` and add your rules:
...
Run the `wtk-generate-project-context` workflow after completing your architecture:
...
For existing projects, run `wtk-generate-project-context` to discover existing patterns:
...
You can edit it manually at any time, or re-run `wtk-generate-project-context` to update it after significant changes.

:::note[File Location]
The default location is `_wtk-output/project-context.md`. Workflows search for it there, and also check `**/project-context.md` anywhere in your project.
:::
