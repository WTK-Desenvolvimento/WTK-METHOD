...
platforms with no shared methodology layer. WTK is uniquely positioned to be the
...
building on the open-source skills CLI ecosystem, WTK offloads the highest-churn
...
```
## Differentiation
- Anti-fragmentation positioning: WTK = cross-platform constant across 40+ fragmenting AI tools; no competitor provides shared methodology layer
- Platform complexity delegated to Vercel skills CLI ecosystem (MIT); WTK maintains methodology, not platform configs
```
...
- **Key gap**: NO competitor combines structured methodology with plugin
  marketplace — this is WTK's whitespace
```
...
**Brief says:**
```
wtk-init must always be included as a base skill in every bundle
```

**Discovery notes say:**
```
wtk-init must always be included as a base skill in every bundle/install
(solves bootstrapping problem)
```

**Distillate keeps the more contextual version:**
```
- wtk-init: always included as base skill in every bundle (solves bootstrapping)
```
...
type: wtk-distillate
sources:
  - "product-brief-wtk-next-gen-installer.md"
  - "product-brief-wtk-next-gen-installer-discovery-notes.md"
downstream_consumer: "PRD creation"
created: "2026-03-13"
token_estimate: 1450
parts: 1
...
## Core Concept
- WTK Next-Gen Installer: replaces monolithic Node.js CLI with skill-based plugin architecture for distributing WTK methodology across 40+ AI platforms
- Three layers: self-describing plugins (wtk-manifest.json), cross-platform install via Vercel skills CLI (MIT), runtime registration via wtk-init skill
- Transforms WTK from dev-only methodology into open platform for any domain (creative, therapeutic, educational, personal)
...
- Plugins: skill bundles with Anthropic plugin standard as base format + wtk-manifest.json extending for WTK-specific metadata (installer options, capabilities, help integration, phase ordering, dependencies)
- Existing manifest example: `{"module-code":"bmm","replaces-skill":"wtk-create-product-brief","capabilities":[{"name":"create-brief","menu-code":"CB","supports-headless":true,"phase-name":"1-analysis","after":["brainstorming"],"before":["create-prd"],"is-required":true}]}`
- Vercel skills CLI handles platform translation; integration pattern (wrap/fork/call) is PRD decision
- wtk-init: global skill scanning installed wtk-manifest.json files, registering capabilities, configuring project settings; always included as base skill in every bundle (solves bootstrapping)
- wtk-update: plugin update path without full reinstall; technical approach (diff/replace/preserve customizations) is PRD decision
...
## Differentiation
- Anti-fragmentation: WTK = cross-platform constant; no competitor provides shared methodology layer across AI tools
- Curated quality: all submissions gated, human-reviewed by Wtk + core team; 13.4% of community skills have critical vulnerabilities (Snyk 2026); quality gate value increases as ecosystem gets noisier
- Domain-agnostic: no competitor builds beyond software dev workflows; same plugin system powers any domain via WTK Builder (separate initiative)
...
- Future plugin creators: non-dev authors using WTK Builder; need distribution without building own installer
...
- wtk-init discovers/registers all plugins from manifests; clear errors for malformed manifests
- At least one external module author successfully publishes plugin using manifest system
- wtk-update works without full reinstall
- Existing CLI users have documented migration path

## Scope
- In: manifest spec, wtk-init, wtk-update, Vercel CLI integration, NPX installer, zip bundles, migration path
- Out: WTK Builder, marketplace web platform, skill conversion (prerequisite, separate), one-click install for all platforms, monetization, quality certification process (gated-submission principle is architectural requirement; process defined separately)
...
- Entry: `tools/cli/wtk-cli.js` (Commander.js) → `tools/cli/installers/lib/core/installer.js`
...
- Skills already use directory-per-skill layout; wtk-manifest.json sidecars exist but are not source of truth
...
- wtk-update mechanics: diff/replace? Preserve user customizations?
...
- wtk-manifest.json as open standard submission to Agent Skills governance?
...
## Opportunities
- Module authors as acquisition channel: each published plugin distributes WTK to creator's audience
- CI/CD integration: wtk-init as pipeline one-liner increases stickiness
- Educational institutions: structured methodology + non-technical install → university AI curriculum
- Skill composability: mixing WTK modules with third-party skills for custom methodology stacks
