---
name: wtk-init
description: "Initialize Wtk project configuration and load config variables. Use when any skill needs module-specific configuration values, or when setting up a new Wtk project."
argument-hint: "[--module=module_code] [--vars=var1:default1,var2] [--skill-path=/path/to/calling/skill]"
---

## Overview

This skill is the configuration entry point for all Wtk skills. It has two modes:
...
Every Wtk skill should call this on activation to get its config vars. The caller never needs to know whether init happened — they just get their config back.

The script `wtk_init.py` is located in this skill's `scripts/` directory. Locate and run it using python for all commands below.

## On Activation — Fast Path

Run the `wtk_init.py` script with the `load` subcommand. Pass `--project-root` set to the project root directory.
...
Run `wtk_init.py` with the `check` subcommand, passing `--module {module_code}`, `--skill-path {calling_skill_path}`, and `--project-root`.
...
- `target_module` — Target module.yaml questions (when module setup needed, discovered from `--skill-path` or `_wtk/{module}/`)
...
1. Resolve defaults by running `wtk_init.py` with the `resolve-defaults` subcommand, passing `--module {module_code}`, `--core-answers '{core_answers_json}'`, and `--project-root`
...
Collect all answers and run `wtk_init.py` with the `write` subcommand, passing `--answers '{all_answers_json}'` and `--project-root`.
...
The script:
- Creates `_wtk/core/config.yaml` with core values (if core answers provided)
- Creates `_wtk/{module}/config.yaml` with core values + module values (result-expanded)
- Creates any directories listed in the module.yaml `directories` array

### Step 5: Return vars

After writing, re-run `wtk_init.py` with the `load` subcommand (same as the fast path) to return resolved vars. Store returned vars as `{var-name}` and return them to the calling skill.
