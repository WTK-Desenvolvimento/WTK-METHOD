...
/**
 * Wtk Method CLI - Direct execution wrapper for npx
...
if (isNpxExecution) {
  ...
  const wtkCliPath = path.join(__dirname, 'cli', 'wtk-cli.js');

  if (!fs.existsSync(wtkCliPath)) {
    console.error('Error: Could not find wtk-cli.js at', wtkCliPath);
  ...
  try {
    ...
    execSync(`node "${wtkCliPath}" ${args.join(' ')}`, {
      stdio: 'inherit',
