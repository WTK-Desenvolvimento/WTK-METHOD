...
class UI {
  ...
  async promptInstall(options = {}) {
    ...
    // Preflight: Check for legacy WTK v4 footprints immediately after getting directory
    ...
    let wtkDir = null;
    let legacyWtkPath = null;

    // First check for legacy .bmad folder (pre-underscore layout)
    ...
    if (await fs.pathExists(confirmedDirectory)) {
      ...
      for (const entry of entries) {
        if (entry.isDirectory() && (entry.name === '.bmad' || entry.name === 'bmad')) {
          ...
          legacyWtkPath = path.join(confirmedDirectory, entry.name);
          wtkDir = legacyWtkPath;

          // Check if it has _cfg folder
          const cfgPath = path.join(legacyWtkPath, '_cfg');
    ...
    // If no .bmad or bmad found, check for current installations _wtk
    if (!hasLegacyBmadFolder) {
      const wtkResult = await installer.findWtkDir(confirmedDirectory);
      wtkDir = wtkResult.wtkDir;
      hasLegacyCfg = wtkResult.hasLegacyCfg;
    }
    ...
    if (hasLegacyBmadFolder || hasLegacyCfg) {
      ...
      await prompts.note(
        'Found a legacy ".bmad"/"bmad" folder, or a legacy "_cfg" folder under the method folder -\n' +
          'this is from an old WTK version that is out of date for automatic upgrade,\n' +
          ...
          '  1. Delete your current WTK installation folder (.bmad or bmad)\n' +
          '  2. Run a fresh installation\n\n' +
          'If you do not want to start fresh, you can attempt to proceed beyond this\n' +
          'point IF you have ensured the method folder is named _wtk, and under it there\n' +
          'is a _config folder. If you have a folder under your method folder named _cfg,\n' +
      ...
      if (proceed === 'cancel') {
        await prompts.note('1. Delete the existing WTK folder in your project\n' + "2. Run 'wtk install' again", 'To do a fresh install');
        process.exit(0);
        return;
      }
      ...
      try {
        // Handle .bmad folder
        if (hasLegacyBmadFolder) {
          const newWtkPath = path.join(confirmedDirectory, '_wtk');
          await fs.move(legacyWtkPath, newWtkPath);
          wtkDir = newWtkPath;
          s.stop(`Renamed "${path.basename(legacyWtkPath)}" to "_wtk"`);
        }

        // Handle _cfg folder (either from .bmad or standalone)
        const cfgPath = path.join(wtkDir, '_cfg');
        if (await fs.pathExists(cfgPath)) {
          s.start('Renaming configuration folder...');
          const newCfgPath = path.join(wtkDir, '_config');
    ...
    // Check if there's an existing WTK installation (after any folder renames)
    const hasExistingInstall = await fs.pathExists(wtkDir);
    ...
    if (hasExistingInstall) {
      ...
      const { existingInstall, wtkDir } = await this.getExistingInstallation(confirmedDirectory);
      const packageJsonPath = path.join(__dirname, '../../../package.json');
      const currentVersion = require(packageJsonPath).version;
      const installedVersion = existingInstall.version || 'unknown';

      // Check if version is pre beta
      const shouldProceed = await this.showLegacyVersionWarning(installedVersion, currentVersion, path.basename(wtkDir), options);
      ...
      // Common actions
      choices.push({ name: 'Modify WTK Installation', value: 'update' });
      ...
      if (actionType === 'update') {
        ...
        if (options.customContent) {
          ...
          if (changeCustomModules) {
            ...
            const { Installer } = require('../installers/lib/core/installer');
            const installer = new Installer();
            const { wtkDir } = await installer.findWtkDir(confirmedDirectory);
  ...
  async promptToolSelection(projectDir, options = {}) {
    // Check for existing configured IDEs - use findWtkDir to detect custom folder names
    const { Detector } = require('../installers/lib/core/detector');
    const { Installer } = require('../installers/lib/core/installer');
    const detector = new Detector();
    const installer = new Installer();
    const wtkResult = await installer.findWtkDir(projectDir || process.cwd());
    const wtkDir = wtkResult.wtkDir;
   ...
   * @returns {Object} Object with existingInstall, installedModuleIds, and wtkDir
   */
  async getExistingInstallation(directory) {
    ...
    const detector = new Detector();
    const installer = new Installer();
    const wtkDirResult = await installer.findWtkDir(directory);
    const wtkDir = wtkDirResult.wtkDir;
    const existingInstall = await detector.detect(wtkDir);
    const installedModuleIds = new Set(existingInstall.modules.map((mod) => mod.id));

    return { existingInstall, installedModuleIds, wtkDir };
  }
  ...
  async selectAllModules(installedModuleIds = new Set()) {
    ...
    allOptions.push({ label: 'Wtk Core Module', value: 'core', hint: 'Core configuration and shared resources' });
    ...
    // Group 2: Wtk Official Modules (type: wtk-org)
  ...
  async displayDirectoryInfo(directory) {
    ...
    if (dirExists) {
      ...
      if (stats.isDirectory()) {
        ...
        if (files.length > 0) {
          // Check for any WTK installation (any folder with _config/manifest.yaml)
          const { Installer } = require('../installers/lib/core/installer');
          const installer = new Installer();
          const wtkResult = await installer.findWtkDir(directory);
          const hasWtkInstall =
            (await fs.pathExists(wtkResult.wtkDir)) && (await fs.pathExists(path.join(wtkResult.wtkDir, '_config', 'manifest.yaml')));

          const wtkNote = hasWtkInstall ? ` including existing WTK installation (${path.basename(wtkResult.wtkDir)})` : '';
          await prompts.log.message(`Directory exists and contains ${files.length} item(s)${wtkNote}`);
        } else {
  ...
  async getConfiguredIdes(directory) {
    ...
    const installer = new Installer();
    const wtkResult = await installer.findWtkDir(directory);
    const existingInstall = await detector.detect(wtkResult.wtkDir);
  ...
  async handleCustomModulesInModifyFlow(directory, selectedModules) {
    ...
    const { Installer } = require('../installers/lib/core/installer');
    const installer = new Installer();
    const { wtkDir } = await installer.findWtkDir(directory);
   ...
   * @param {string} wtkFolderName - Name of the WTK folder
   * @returns {Promise<boolean>} True if user wants to proceed, false if they cancel
   */
  async showLegacyVersionWarning(installedVersion, currentVersion, wtkFolderName, options = {}) {
    ...
    let warningContent;
    if (installedVersion === 'unknown') {
      warningContent = 'Unable to detect your installed WTK version.\n' + 'This appears to be a legacy or unsupported installation.';
    } else {
      warningContent =
    ...
    warningContent +=
      ...
      '  1. Delete your current WTK installation folder\n' +
      `     (the "${wtkFolderName}/" folder in your project)\n` +
      '  2. Run a fresh installation\n\n' +
      'Benefits of a fresh install:\n' +
      '  \u2022 Cleaner configuration without legacy artifacts\n' +
      '  \u2022 All new features properly configured\n' +
    ...
    if (proceed === 'cancel') {
      await prompts.note(
        `1. Delete the "${wtkFolderName}/" folder in your project\n` + "2. Run 'wtk install' again",
  ...
  async displayStatus(statusData) {
    const { installation, modules, availableUpdates, wtkDir } = statusData;
    ...
    if (availableUpdates.length > 0) {
      ...
      await prompts.log.message('Run \'wtk install\' and select "Quick Update" to update');
    } else {
