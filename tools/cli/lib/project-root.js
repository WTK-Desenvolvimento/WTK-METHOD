...
/**
 * Find the WTK project root directory by looking for package.json
 * or specific WTK markers
 */
function findProjectRoot(startPath = __dirname) {
  let currentPath = path.resolve(startPath);

  // Keep going up until we find package.json with wtk-method
  while (currentPath !== path.dirname(currentPath)) {
    ...
    if (fs.existsSync(packagePath)) {
      try {
        ...
        // Check if this is the WTK project
        if (pkg.name === 'wtk-method' || fs.existsSync(path.join(currentPath, 'src', 'core-skills'))) {
          return currentPath;
        }
      } catch {
        // Continue searching
...
function getModulePath(moduleName, ...segments) {
  ...
  if (moduleName === 'n8n') {
    return getSourcePath('n8n-skills', ...segments);
  }
  if (moduleName === 'linear') {
    return getSourcePath('linear-skills', ...segments);
  }
