  ...
  try {
    ...
    if (result && semver.gt(result, packageJson.version)) {
      ...
      const updateMsg = [
        ...
        `  npm cache clean --force && npx wtk-method@${tag} install`,
      ].join('\n');
...
// Set up main program
program.version(packageJson.version).description('WTK Core CLI - Universal AI agent framework');
