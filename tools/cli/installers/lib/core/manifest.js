...
class Manifest {
   ...
   */
  async create(wtkDir, data, installedFiles = []) {
    ...
    // Get the Wtk version from package.json
    const wtkVersion = data.version || require(path.join(process.cwd(), 'package.json')).version;
    ...
    if (data.modules && Array.isArray(data.modules)) {
      for (const moduleName of data.modules) {
        ...
        const moduleVersion =
          moduleName === 'core' || moduleName === 'bmm' || moduleName === 'n8n' || moduleName === 'linear' ? wtkVersion : null;
        ...
        moduleDetails.push({
          ...
          installDate: now,
          lastUpdated: now,
          source: moduleName === 'core' || moduleName === 'bmm' || moduleName === 'n8n' || moduleName === 'linear' ? 'built-in' : 'unknown',
        });
      }
    }
   ...
   */
  async read(wtkDir) {
    const yamlPath = path.join(wtkDir, '_config', 'manifest.yaml');
    const yaml = require('yaml');
   ...
   */
  async update(wtkDir, updates, installedFiles = null) {
    const yaml = require('yaml');
    const manifest = (await this._readRaw(wtkDir)) || {
      installation: {},
      modules: [],
      ides: [],
    };
   ...
   */
  async addModule(wtkDir, moduleName, options = {}) {
    let manifest = await this._readRaw(wtkDir);
    if (!manifest) {
    ...
    await this._writeRaw(wtkDir, manifest);
  }
   ...
   */
  async removeModule(wtkDir, moduleName) {
    const manifest = await this._readRaw(wtkDir);
   ...
   */
  async updateModuleVersion(wtkDir, moduleName, versionInfo) {
    const manifest = await this._readRaw(wtkDir);
    if (!manifest || !manifest.modules) {
      return;
    }
   ...
   */
  async getModuleVersion(wtkDir, moduleName) {
    const manifest = await this._readRaw(wtkDir);
    if (!manifest || !manifest.modules) {
      return null;
    }
   ...
   */
  async getAllModuleVersions(wtkDir) {
    const manifest = await this._readRaw(wtkDir);
    if (!manifest || !manifest.modules) {
      return [];
    }
   ...
   */
  async _writeRaw(wtkDir, manifestData) {
    const yaml = require('yaml');
    const manifestPath = path.join(wtkDir, '_config', 'manifest.yaml');
   ...
   */
  async addIde(wtkDir, ideName) {
    const manifest = await this.read(wtkDir);
    if (!manifest) {
    ...
    if (!manifest.ides.includes(ideName)) {
      ...
      await this.update(wtkDir, { ides: manifest.ides });
    }
  }
   ...
   * @param {string} wtkDir - Path to bmad directory for relative paths
   * @returns {Array} Array of file metadata objects
   */
  async parseInstalledFiles(installedFiles, wtkDir) {
    const fileMetadata = [];

    for (const filePath of installedFiles) {
      const fileExt = path.extname(filePath).toLowerCase();
      // Make path relative to parent of bmad directory, starting with 'bmad/'
      const relativePath = '_wtk' + filePath.replace(wtkDir, '').replaceAll('\\', '/');
   ...
   */
  async addCustomModule(wtkDir, customModule) {
    const manifest = await this.read(wtkDir);
    if (!manifest) {
      throw new Error('No manifest found');
    }
    ...
    await this.update(wtkDir, { customModules: manifest.customModules });
  }
   ...
   */
  async removeCustomModule(wtkDir, moduleId) {
    const manifest = await this.read(wtkDir);
   ...
   */
  async getModuleVersionInfo(moduleName, wtkDir, moduleSourcePath = null) {
    const os = require('node:os');
    const yaml = require('yaml');

    // Built-in modules use Wtk version (core, bmm, and n8n ship in the WTK-METHOD repo)
    if (['core', 'bmm', 'n8n', 'linear'].includes(moduleName)) {
    ...
    const cacheDir = path.join(wtkDir, '_config', 'custom', moduleName);
   ...
   */
  async checkForUpdates(wtkDir) {
    const modules = await this.getAllModuleVersions(wtkDir);
    const updates = [];
