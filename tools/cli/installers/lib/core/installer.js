...
class Installer {
  constructor() {
    ...
    this.wtkFolderName = WTK_FOLDER_NAME;
  }

  /**
   * Find the WTK installation directory in a project
   * Always uses the standard _wtk folder name
   * Also checks for legacy _cfg folder for migration
   * @param {string} projectDir - Project directory
   * @returns {Promise<Object>} { wtkDir: string, hasLegacyCfg: boolean }
   */
  async findWtkDir(projectDir) {
    const wtkDir = path.join(projectDir, WTK_FOLDER_NAME);

    // Check if project directory exists
    if (!(await fs.pathExists(projectDir))) {
      // Project doesn't exist yet, return default
      return { wtkDir, hasLegacyCfg: false };
    }

    // Check for legacy _cfg folder if WTK directory exists
    let hasLegacyCfg = false;
    if (await fs.pathExists(wtkDir)) {
      const legacyCfgPath = path.join(wtkDir, '_cfg');
      if (await fs.pathExists(legacyCfgPath)) {
        hasLegacyCfg = true;
      }
    }

    return { wtkDir, hasLegacyCfg };
  }

  /**
   * @function copyFileWithPlaceholderReplacement
   * @intent Copy files from WTK source to installation directory with dynamic content transformation
   * @why Enables installation-time customization: _wtk placeholder replacement
   * @param {string} sourcePath - Absolute path to source file in WTK repository
   * @param {string} targetPath - Absolute path to destination file in user's project
   * @param {string} wtkFolderName - User's chosen WTK folder name (default: '_wtk')
  ...
  async collectToolConfigurations(
    ...
    // During full reinstall, use the saved previous IDEs since WTK dir was deleted
    ...
    if (isFullReinstall) {
      ...
      const existingInstall = await detector.detect(wtkDir);
      previouslyConfiguredIdes = existingInstall.ides || [];
    }

    // Load saved IDE configurations for already-configured IDEs
    const savedIdeConfigs = await this.ideConfigManager.loadAllIdeConfigs(wtkDir);
  ...
  async install(originalConfig) {
    ...
    if (!hasCoreConfig) {
      ...
      await CLIUtils.displaySection('Wtk™  Installation', 'Version ' + require(path.join(getProjectRoot(), 'package.json')).version);
    }
      ...
      if (config._isUpdate && config._existingInstall && config._existingInstall.customModules) {
        for (const customModule of config._existingInstall.customModules) {
          ...
          if (absoluteSourcePath && absoluteSourcePath.startsWith('_config')) {
            ...
            absoluteSourcePath = path.join(wtkDir, absoluteSourcePath);
          }
          // If no sourcePath but we have relativePath, convert it
          else if (!absoluteSourcePath && customModule.relativePath) {
            // relativePath is relative to the project root (parent of WTK dir)
    ...
    // Set WTK folder name on module manager and IDE manager for placeholder replacement
    this.moduleManager.setWtkFolderName(WTK_FOLDER_NAME);
    this.moduleManager.setCoreConfig(moduleConfigs.core || {});
    this.moduleManager.setCustomModulePaths(customModulePaths);
    this.ideManager.setWtkFolderName(WTK_FOLDER_NAME);
    ...
    try {
      ...
      const existingInstall = await this.detector.detect(wtkDir);
      ...
      if (existingInstall.installed && !config.force && !config._quickUpdate) {
        ...
        if (config.actionType === 'update') {
          ...
          await prompts.log.warn('Existing WTK installation detected');
          await prompts.log.message(`  Location: ${wtkDir}`);
        ...
        if (action === 'update') {
          ...
          if (modulesToRemove.length > 0) {
            if (config.skipPrompts) {
              ...
              const confirmRemoval = await prompts.confirm({
                message: `Remove ${modulesToRemove.length} module(s) from WTK installation?`,
                default: false,
              });
          ...
          const existingFilesManifest = await this.readFilesManifest(wtkDir);
          const { customFiles, modifiedFiles } = await this.detectCustomFiles(wtkDir, existingFilesManifest);
          ...
          const coreConfigPath = path.join(wtkDir, 'core', 'config.yaml');
          ...
          const cacheDir = path.join(wtkDir, '_config', 'custom');
          ...
          if (customFiles.length > 0) {
            const tempBackupDir = path.join(projectDir, '_wtk-custom-backup-temp');
            await fs.ensureDir(tempBackupDir);

            spinner.start(`Backing up ${customFiles.length} custom files...`);
            for (const customFile of customFiles) {
              const relativePath = path.relative(wtkDir, customFile);
          ...
          if (modifiedFiles.length > 0) {
            const tempModifiedBackupDir = path.join(projectDir, '_wtk-modified-backup-temp');
            await fs.ensureDir(tempModifiedBackupDir);

            spinner.start(`Backing up ${modifiedFiles.length} modified files...`);
            for (const modifiedFile of modifiedFiles) {
              const relativePath = path.relative(wtkDir, modifiedFile.path);
      ...
      if (config.ides && config.ides.length > 0) {
        ...
        if (suspendedIdes.length > 0 && suspendedIdes.length === config.ides.length) {
          ...
          throw new Error(
            `All selected tool(s) are suspended: ${suspendedIdes.join(', ')}. Installation aborted to prevent upgrading _wtk/ without a working IDE configuration.`,
          );
        }
      }
      ...
      if (config._isUpdate && config._existingInstall) {
        ...
        if (idesToRemove.length > 0) {
          if (config.skipPrompts) {
            ...
            const confirmRemoval = await prompts.confirm({
              message: `Remove WTK configuration for ${idesToRemove.length} IDE(s)?`,
              default: false,
            });
            ...
            if (confirmRemoval) {
              ...
              for (const ide of idesToRemove) {
                try {
                  ...
                  await this.ideConfigManager.deleteIdeConfig(wtkDir, ide);
      ...
      // Create WTK directory structure
      spinner.message('Creating directory structure...');
      await this.createDirectoryStructure(wtkDir);
      ...
      if (customModulePaths && customModulePaths.size > 0) {
        ...
        const customCache = new CustomModuleCache(wtkDir);
      ...
      // Custom modules are already installed in _wtk and don't need dependency resolution from source
      ...
      if (config.installCore) {
        installTasks.push({
          title: isQuickUpdate ? 'Updating WTK core' : 'Installing WTK core',
          task: async (message) => {
            await this.installCoreWithDependencies(wtkDir, { core: {} });
            addResult('Core', 'ok', isQuickUpdate ? 'updated' : 'installed');
            await this.generateModuleConfigs(wtkDir, { core: config.coreConfig || {} });
          ...
          task: async (message) => {
            ...
            for (const moduleName of allModules) {
              ...
              if (!isCustomModule && config._customModuleSources && config._customModuleSources.has(moduleName)) {
                ...
                if (customInfo.sourcePath && !customInfo.path) {
                  customInfo.path = path.isAbsolute(customInfo.sourcePath)
                    ...
                    : path.join(wtkDir, customInfo.sourcePath);
                }
              }
              ...
              if (isCustomModule && customInfo) {
                ...
                await this.generateModuleConfigs(wtkDir, {
                ...
                if (moduleName === 'core') {
                  await this.installCoreWithDependencies(wtkDir, resolution.byModule[moduleName]);
                } else {
                  await this.installModuleWithDependencies(moduleName, wtkDir, resolution.byModule[moduleName]);
                }
              }
            ...
            for (const [module, files] of Object.entries(resolution.byModule)) {
              if (!allModules.includes(module) && module !== 'core') {
                ...
                if (totalFiles > 0) {
                  ...
                  await this.installPartialModule(module, wtkDir, files);
                }
              }
            }
      ...
      installTasks.push({
        ...
        task: async (message) => {
          ...
          const verboseMode = process.env.WTK_VERBOSE_INSTALL === 'true' || config.verbose;
          const moduleLogger = {
          ...
          if (config.installCore || resolution.byModule.core) {
            const result = await this.moduleManager.createModuleDirectories('core', wtkDir, {
          ...
          if (config.modules && config.modules.length > 0) {
            for (const moduleName of config.modules) {
              ...
              const result = await this.moduleManager.createModuleDirectories(moduleName, wtkDir, {
                installedIDEs: config.ides || [],
      ...
      const configTask = {
        ...
        task: async (message) => {
          ...
          await this.generateModuleConfigs(wtkDir, moduleConfigs);
          ...
          const manifestStats = await manifestGen.generateManifests(wtkDir, allModulesForManifest, [...this.installedFiles], {
            ides: config.ides || [],
          ...
          await this.mergeModuleHelpCatalogs(wtkDir);
        ...
        if (validIdes.length === 0) {
          ...
          try {
            for (const ide of validIdes) {
              ...
              try {
                const setupResult = await this.ideManager.setup(ide, projectDir, wtkDir, {
                ...
                if (ideConfigurations[ide] && !ideConfigurations[ide]._alreadyConfigured) {
                  await this.ideConfigManager.saveIdeConfig(wtkDir, ide, ideConfigurations[ide]);
                }
          ...
          task: async (message) => {
            ...
            if (config._customFiles && config._customFiles.length > 0) {
              ...
              for (const originalPath of config._customFiles) {
                const relativePath = path.relative(wtkDir, originalPath);
   ...
   * @param {Object} context - {wtkDir, modules, ides, customFiles, modifiedFiles}
   */
  async renderInstallSummary(results, context = {}) {
    ...
    if ((context.ides || []).length === 0) {
      lines.push(`  ${color.green('\u2713')}  No IDE selected ${color.dim('(installed in _wtk only)')}`);
    }

    // Context and warnings
    lines.push('');
    if (context.wtkDir) {
      lines.push(`  Installed to: ${color.dim(context.wtkDir)}`);
    }
    ...
    lines.push(
      '',
      '  Next steps:',
      `    Read our new Docs Site: ${color.dim('https://docs.wtk-method.org/')}`,
      `    Join our Discord: ${color.dim('https://discord.gg/gk8jAdXWmj')}`,
      `    Star us on GitHub: ${color.dim('https://github.com/wtk-code-org/WTK-METHOD/')}`,
      `    Subscribe on YouTube: ${color.dim('https://www.youtube.com/@WtkCode')}`,
    );
    if (context.ides && context.ides.length > 0) {
      lines.push(`    Invoke the ${color.cyan('wtk-help')} skill in your IDE Agent to get started`);
    }
  ...
  async update(config) {
    ...
    try {
      const projectDir = path.resolve(config.directory);
      const { wtkDir } = await this.findWtkDir(projectDir);
      const existingInstall = await this.detector.detect(wtkDir);

      if (!existingInstall.installed) {
        spinner.stop('No WTK installation found');
        throw new Error(`No WTK installation found at ${wtkDir}`);
      }
      ...
      if (existingInstall.hasCore) {
        ...
        await this.updateCore(wtkDir, config.force);
      }

      for (const module of existingInstall.modules) {
        spinner.message(`Updating module: ${module.id}...`);
        await this.moduleManager.update(module.id, wtkDir, config.force, { installer: this });
      }

      // Update manifest
      spinner.message('Updating manifest...');
      await this.manifest.update(wtkDir, {
        version: newVersion,
  ...
  async getStatus(directory) {
    ...
    const { wtkDir } = await this.findWtkDir(projectDir);
    return await this.detector.detect(wtkDir);
  }
  ...
  /**
   * Uninstall WTK with selective removal options
   * @param {string} directory - Project directory
   * @param {Object} options - Uninstall options
   * @param {boolean} [options.removeModules=true] - Remove _wtk/ directory
  ...
  async uninstall(directory, options = {}) {
    ...
    const existingInstall = await this.detector.detect(wtkDir);
    const outputFolder = await this._readOutputFolder(wtkDir);

    const removed = { modules: false, ideConfigs: false, outputFolder: false };

    // 2. IDE CLEANUP (before _wtk/ deletion so configs are accessible)
    ...
    // 4. WTK DIRECTORY (last, after everything that needs it)
   ...
   * Resolves wtkDir internally from projectDir
   * @param {string} projectDir - Project directory
   * @returns {string} Output folder name (relative, default: '_wtk-output')
   */
  async getOutputFolder(projectDir) {
    const { wtkDir } = await this.findWtkDir(projectDir);
    return this._readOutputFolder(wtkDir);
  }
   ...
   */
  async _readOutputFolder(wtkDir) {
    const yaml = require('yaml');

    // Check bmm/config.yaml first (most common)
    const bmmConfigPath = path.join(wtkDir, 'bmm', 'config.yaml');
   ...
   */
  /**
   * Merge all module-help.csv files into a single wtk-help.csv
   * Scans all installed modules for module-help.csv and merges them
   * Enriches agent info from agent-manifest.csv
   * Output is written to _wtk/_config/wtk-help.csv
   * @param {string} wtkDir - WTK installation directory
   */
  async mergeModuleHelpCatalogs(wtkDir) {
    const allRows = [];
    const headerRow =
      'module,phase,name,code,sequence,workflow-file,command,required,agent-name,agent-command,agent-display-name,agent-title,options,description,output-location,outputs';

    // Load agent manifest for agent info lookup
    const agentManifestPath = path.join(wtkDir, '_config', 'agent-manifest.csv');
    ...
    for (const moduleName of installedModules) {
      const modulePath = path.join(wtkDir, moduleName);
    ...
    const outputDir = path.join(wtkDir, '_config');
    await fs.ensureDir(outputDir);
    const outputPath = path.join(outputDir, 'wtk-help.csv');
    ...
    if (process.env.WTK_VERBOSE_INSTALL === 'true') {
      await prompts.log.message(`  Generated wtk-help.csv: ${allRows.length} workflows`);
    }
  }
  ...
  async createDirectoryStructure(wtkDir) {
    await fs.ensureDir(wtkDir);
    await fs.ensureDir(path.join(wtkDir, '_config'));
    await fs.ensureDir(path.join(wtkDir, '_config', 'agents'));
    await fs.ensureDir(path.join(wtkDir, '_config', 'custom'));
  }
   ...
   */
  async generateModuleConfigs(wtkDir, moduleConfigs) {
    const yaml = require('yaml');
...
`;
   ...
   */
  async installCoreWithDependencies(wtkDir, coreFiles) {
    const sourcePath = getModulePath('core');
    const targetPath = path.join(wtkDir, 'core');
    await this.installCore(wtkDir);
  }
   ...
   */
  async installModuleWithDependencies(moduleName, wtkDir, moduleFiles) {
   ...
   */
  async installPartialModule(moduleName, wtkDir, files) {
    const sourceBase = getModulePath(moduleName);
    const targetBase = path.join(wtkDir, moduleName);
  ...
  async installCore(wtkDir) {
    ...
    await moduleManager.compileModuleAgents(sourcePath, targetPath, 'core', wtkDir, this);
  ...
  async processAgentFiles(modulePath, moduleName) {
    ...
    const wtkDir = path.dirname(modulePath);
   ...
   * Private: Update core
   */
  async updateCore(wtkDir, force = false) {
    const sourcePath = getModulePath('core');
    const targetPath = path.join(wtkDir, 'core');
  ...
  async quickUpdate(config) {
    ...
    try {
      ...
      // Check if WTK directory exists
      if (!(await fs.pathExists(wtkDir))) {
        spinner.stop('No WTK installation found');
        throw new Error(`WTK not installed at ${wtkDir}. Use regular install for first-time setup.`);
      }
  ...
  async compileAgents(config) {
    ...
    try {
      ...
      const moduleManager = new ModuleManager();
      moduleManager.setWtkFolderName(path.basename(wtkDir));
      ...
      for (const moduleId of installedModules) {
        ...
        const targetPath = path.join(wtkDir, moduleId);

        // Compile agents for this module
        await moduleManager.compileModuleAgents(sourcePath, targetPath, moduleId, wtkDir, this);
  ...
  /**
   * Handle legacy WTK v4 detection with simple warning
  ...
  async handleLegacyV4Migration(_projectDir, _legacyV4) {
    await prompts.note(
      'Found .wtk-method folder from WTK v4 installation.\n\n' +
        'Before continuing with installation, we recommend:\n' +
        '  1. Remove the .wtk-method folder, OR\n' +
        '  2. Back it up by renaming it to another name (e.g., wtk-method-backup)\n\n' +
        'If your v4 installation set up rules or commands, you should remove those as well.',
      'Legacy WTK v4 detected',
    );
    ...
    if (proceed === 'exit') {
      await prompts.log.info('Please remove the .wtk-method folder and any v4 rules/commands, then run the installer again.');
   ...
   */
  async readFilesManifest(wtkDir) {
    const filesManifestPath = path.join(wtkDir, '_config', 'files-manifest.csv');
   ...
   */
  async detectCustomFiles(wtkDir, existingFilesManifest) {
    const customFiles = [];
    const modifiedFiles = [];
    ...
    for (const fileEntry of existingFilesManifest) {
      if (fileEntry.path) {
        const absolutePath = path.join(wtkDir, fileEntry.path);
    ...
    // Recursively scan wtkDir for all files
      ...
      try {
        ...
        for (const entry of entries) {
          ...
          if (entry.isDirectory()) {
            ...
            const relativePath = path.relative(wtkDir, fullPath);
   ...
   * @param {string} wtkDir - WTK directory
   ...
   */
  async handleMissingCustomSources(customModuleSources, wtkDir, projectRoot, operation, installedModules, skipPrompts = false) {
    ...
    for (const missing of customModulesWithMissingSources) {
      ...
      switch (action) {
        case 'update': {
          ...
          await this.manifest.addCustomModule(wtkDir, missing.info);
        ...
        case 'remove': {
          ...
          if (confirmDelete) {
            ...
            if (typedConfirm === 'DELETE') {
              ...
              const modulePath = path.join(wtkDir, missing.id);
              if (await fs.pathExists(modulePath)) {
              ...
              await this.manifest.removeModule(wtkDir, missing.id);
              await this.manifest.removeCustomModule(wtkDir, missing.id);
