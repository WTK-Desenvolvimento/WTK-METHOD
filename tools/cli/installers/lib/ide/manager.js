...
class IdeManager {
  ...
  setWtkFolderName(wtkFolderName) {
    ...
    for (const handler of this.handlers.values()) {
      if (typeof handler.setWtkFolderName === 'function') {
        handler.setWtkFolderName(wtkFolderName);
      }
    }
  }
  ...
  async loadConfigDrivenHandlers() {
    ...
    for (const [platformCode, platformInfo] of Object.entries(platformConfig.platforms)) {
      ...
      if (typeof handler.setWtkFolderName === 'function') {
        handler.setWtkFolderName(this.wtkFolderName);
      }
   ...
   */
  async setup(ideName, projectDir, wtkDir, options = {}) {
    ...
    try {
      const handlerResult = await handler.setup(projectDir, wtkDir, options);
