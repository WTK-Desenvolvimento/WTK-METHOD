...
module.exports = {
  command: 'uninstall',
  description: 'Remove WTK installation from the current project',
  options: [
    ['-y, --yes', 'Remove all WTK components without prompting (preserves user artifacts)'],
    ...
    try {
      ...
      if (options.directory) {
        ...
        const dirChoice = await prompts.select({
          message: 'Where do you want to uninstall WTK from?',
          choices: [
      ...
      const { wtkDir } = await installer.findWtkDir(projectDir);

      if (!(await fs.pathExists(wtkDir))) {
        await prompts.log.warn('No WTK installation found.');
        process.exit(0);
      }
      ...
      await prompts.intro('WTK Uninstall');
      ...
      if (!options.yes) {
        ...
        const selected = await prompts.multiselect({
          ...
          options: [
            {
              value: 'modules',
              label: `WTK Modules & data (${installer.wtkFolderName}/)`,
      ...
      // Phase 3: WTK modules & data (last — other phases may need _wtk/)
      if (removeModules) {
        const s = await prompts.spinner();
        s.start(`Removing WTK modules & data (${installer.wtkFolderName}/)...`);
      ...
      await prompts.outro('To reinstall, run: npx wtk-method install');
