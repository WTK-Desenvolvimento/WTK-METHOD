 ...
 *
 * Validates cross-file references in WTK source files (agents, workflows, tasks, steps).
 * Catches broken file paths, missing referenced files, and absolute path leaks.
 *
 * What it checks:
 * - {project-root}/_wtk/ references in YAML and markdown resolve to real src/ files
...
// Pattern: {project-root}/_wtk/ references
const PROJECT_ROOT_REF = /\{project-root\}\/_wtk\/([^\s'"<>})\]`]+)/g;

// Pattern: {_wtk}/ shorthand references
const WTK_SHORTHAND_REF = /\{_wtk\}\/([^\s'"<>})\]`]+)/g;
...
function mapInstalledToSource(refPath) {
  // Strip {project-root}/_wtk/ or {_wtk}/ prefix
  let cleaned = refPath.replace(/^\{project-root\}\/_wtk\//, '').replace(/^\{_wtk\}\//, '');

  // Also handle bare _wtk/ prefix (seen in some invoke-task)
  cleaned = cleaned.replace(/^_wtk\//, '');
...
function isInstallOnly(cleanedPath) {
  // Skip paths that only exist in the installed _wtk/ structure, not in src/
...
function extractYamlRefs(filePath, content) {
  ...
  function checkValue(value, range, keyPath) {
    ...
    // Check for {project-root}/_wtk/ refs
    const prMatch = value.match(/\{project-root\}\/_wtk\/[^\s'"<>})\]`]+/);
    if (prMatch) {
      refs.push({ file: filePath, raw: prMatch[0], type: 'project-root', line, key: keyPath });
    }

    // Check for {_wtk}/ refs
    const bmMatch = value.match(/\{_wtk\}\/[^\s'"<>})\]`]+/);
    if (bmMatch) {
...
function extractMarkdownRefs(filePath, content) {
  ...
  // {project-root}/_wtk/ refs
  runPattern(PROJECT_ROOT_REF, 'project-root');

  // {_wtk}/ shorthand
  runPattern(WTK_SHORTHAND_REF, 'project-root');
...
function resolveRef(ref) {
  ...
  if (ref.type === 'exec-attr') {
    ...
    if (execPath.includes('{_wtk}')) {
      return mapInstalledToSource(execPath);
    }
    if (execPath.startsWith('_wtk/')) {
      return mapInstalledToSource(execPath);
    }
    // Relative exec path
  ...
  if (ref.type === 'invoke-task') {
    ...
    const prMatch = ref.raw.match(/\{project-root\}\/_wtk\/([^\s'"<>})\]`]+)/);
    if (prMatch) return mapInstalledToSource(prMatch[0]);

    const bmMatch = ref.raw.match(/\{_wtk\}\/([^\s'"<>})\]`]+)/);
    if (bmMatch) return mapInstalledToSource(bmMatch[0]);

    const bareMatch = ref.raw.match(/_wtk\/([^\s'"<>})\]`]+)/);
